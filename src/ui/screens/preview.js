...
function _pvTokenSwatchCard(token, label) {
  ...
  const hexEl = el(
    "div",
    { class: "absolute inset-0 flex items-center justify-center transition-opacity duration-150", style: "opacity:0" },
  ...
  return el("div", { class: "flex flex-col gap-1" }, [
    swatch,
    el("div", { class: "text-[12px] font-medium leading-none text-center", style: "color:var(--pv-muted)" }, label),
  ]);
}

function _pvRoleGroup(roleIdx, variations) {
  ...
  for (const [varKey, token] of Object.entries(variations)) {
    grid.appendChild(_pvTokenSwatchCard(token, _pvVarLabel(varKey)));
  }
...
function _pvColorGroup(colorName, roles, result) {
  ...
  for (const [roleIdx, variations] of Object.entries(roles)) {
    content.appendChild(_pvRoleGroup(roleIdx, variations));
  }
  return el("div", { class: "mb-4 px-2 pt-2" }, [
...
function renderThemePanel(panelOrId, themeTokens, bgHex, result) {
  ...
  for (const [colorName, roles] of Object.entries(themeTokens)) {
    panelEl.appendChild(_pvColorGroup(colorName, roles, result));
  }
}
